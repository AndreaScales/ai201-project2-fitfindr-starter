...
def handle_query(user_query: str, wardrobe_choice: str) -> tuple[str, str, str]:
    ...
    # Step 4: If the agent ended early with an error, show it in the first
    # panel and leave the other two blank.
    if session["error"]:
        return session["error"], "", ""

    # Step 5: Format the selected listing for display, then return it alongside
    # the outfit suggestion and the fit card.
    listing_text = _format_listing(session["selected_item"])
    return listing_text, session["outfit_suggestion"], session["fit_card"]


def _format_listing(item: dict) -> str:
    """Render a listing dict as readable text for the 'Top listing found' panel."""
